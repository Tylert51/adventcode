...
public class Sol {
    public static void main(String[] args) {
        ...
        int sum = 0;
        for (int[] coord : possibArr) {
            for (Number num : numbers) {
                if (doesContain(num.getCoords(), coord) && !doesContain(usedCoords, coord)) {
                    partNums.add(num.getValue());
                    for (int[] coo : num.getCoords()) {
                        usedCoords.add(coo);
                    }
                }
            }
        ...
        for (Number num : numbers) {
            for (int[] coord : num.getCoords()) {
                if (doesContain(possibArr, coord)) {
                    partNums.add(num.getValue());
                    break;
                }
            }
